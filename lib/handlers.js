                    ...
                    if (!err && data) {
                        ...
                        _data.delete('users', email, function(err) {
                            if (!err) {
                                ...
                                callback(
                                    httpStates.CANT_DELETE_FILE.statusCode,
                                    {
                                        Error:
                                            httpStates.CANT_DELETE_FILE
                                                .message +
                                            ', Error: ' +
                                            err
                                    }
...
handlers._cart.post = function(data, callback) {
    var inventoryId =
        typeof data.payload.inventoryId == 'number'
            ? data.payload.inventoryId
            : 0;
    var quantity =
    ...
    if (inventoryId && quantity) {
        ...
        _data.read('tokens', token, function(err, tokenData) {
            if (!err && tokenData) {
                ...
                handlers._tokens.verifyToken(token, email, function(
                    isTokenValid
                ) {
                    if (isTokenValid) {
                        // Get user data
